 ...
 * with an additional Node* parameter to allow for recursive calls.
 * @param subRoot
 * @return The height of the subtree.
 */
template <typename T>
 ...
 * with an additional Node* parameter to allow for recursive calls.
 * @param subRoot
 * @return The sum of distances of all nodes to the root
 */
template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot, int depth) const { 
  ...
  if (subRoot == NULL)
      return 0;
  return sumDistances(subRoot->left, depth+1) + sumDistances(subRoot->right, depth+1) + depth;
}
