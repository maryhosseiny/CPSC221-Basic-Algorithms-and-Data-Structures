...
private:
    ...
    */
    ...
    // void copyNode(Node * currNode, Node * otherNode);
    
    
    

    void clear(Node* curr);
    void pruneHelper(Node* root, const int tol);
    void renderHelper(PNG& image, Node* node) const;
    void copyNode(Node*& currNode, Node * otherNode);
    void clearSubTrees(Node * root);
    int pruneSizeHelper(Node* node, const int tol) const;
    int idealPruneRecursive(int const leaves, int minTolerance, int maxTolerance) const;
};
